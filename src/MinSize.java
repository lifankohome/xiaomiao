...
public class MinSize implements ActionListener {
    ...
    public static void create() {
        if (SystemTray.isSupported()) {// 判断是否支持系统托盘
            ...
            URL url = MinSize.class.getResource("icon.png");// 获取图片所在的URL
            ...
            trayIcon.setToolTip("状态：已连接\r\n姓名：" + Network.name + "\r\n学号：" + Network.stuId);// 添加工具提示文本

            // 创建弹出菜单
            PopupMenu popupMenu = new PopupMenu();

            MenuItem main = new MenuItem("打开监控界面");
            MenuItem login = new MenuItem("我的在线时长");
            MenuItem IDP = new MenuItem("创新开发平台");
            MenuItem list = new MenuItem("在线时长排名");
            ...
            login.addActionListener(e -> JOptionPane.showMessageDialog(null, "<html>" + Network.timeOnline + "<html>", "我的在线时长",
                    JOptionPane.INFORMATION_MESSAGE));
            list.addActionListener(e -> Login.openURL("http://eeec.hpu.edu.cn/idp/xiaoMiao/record.php"));
            IDP.addActionListener(e -> Login.openURL("http://eeec.hpu.edu.cn/idp/"));
